...
public class MessageHandler {
    ...
    public void registerService(ServiceRegistrationMessage msg){
        ...
        messageHandlerPresenter.printToLog("Service möchte sich registrieren.");
        if(handlingSW != null && handlingSW.isUpTpDate()){
            messageHandlerPresenter.printToLog("Notwendige Software bereits installiert, schlage den Service vor...");
            mmsConnection.sendMessage(msg);
        } else{
            messageHandlerPresenter.printToLog("Service Provider benötigt neue Software! InquiryID: "+ msg.getInquiryID()
    ...
    public void waitForSoftwarePackage(SoftwareInstallationPackage installationPackage){
        if(installationPackage.getUpdatedManifest().equals(getManifest())){
            ...
            //Neue Software
            messageHandlerPresenter.printToLog("Neue Software vom Server erhalten: "+installationPackage.getSoftware().getDescription().getTitle());
